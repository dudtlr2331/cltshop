...
/**
 * 작성자 : 일자 : 2021. 01. 14. select, insert, update, delete 쿼리 java Object를
 * 테이블 이름으로 파라미터를 받아 생성한다.
 */
public class OracleDataBuilder {
	private static final String DRIVER_CLASS_NAME 	= "oracle.jdbc.driver.OracleDriver";
	private static final String URL 				= "jdbc:oracle:thin:@127.0.0.1:1521:xe";
	private static final String USER_NAME 			= "hangang";
	private static final String PASS_WORD 			= "victory123";

	private static final String TARGET_TABLE 		= "GOODS_INFO";

	public static void main(String[] args) throws IOException {
		String driver = DRIVER_CLASS_NAME;
		String url = URL;
		String userName = USER_NAME;
		String userPass = PASS_WORD;

		System.out.println("driver : " + driver + "\nurl : " + url + "\nuserName : " + userName + "\nuserPass : " + userPass);

		OracleDataBuilder obj = new OracleDataBuilder(driver, url, userName, userPass);
...
//		result = obj.createJavaObject(TARGET_TABLE);
	...
	private OracleDataBuilder(String driver, String url, String userName, String userPass) {
		this.driver = driver;
		this.url = url;
	...
	public String createJavaObject(String tableName) {
		...
		try {
			this.getConnect();
			String sql = "SELECT * FROM information_schema.columns WHERE table_schema = \'" + "\'  AND TABLE_NAME = \'" + tableName + "\'";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
